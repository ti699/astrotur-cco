-- =====================================================
-- ⚠️  ATENÇÃO: ESTE ARQUIVO É PARA PostgreSQL LOCAL
-- =====================================================
-- Este arquivo contém comandos do psql que NÃO funcionam
-- no Supabase SQL Editor.
--
-- 🚫 NÃO USE este arquivo no Supabase!
-- ✅ USE: schema-supabase.sql para Supabase
--
-- Este arquivo é para instalação local do PostgreSQL.
-- =====================================================
...
CREATE INDEX idx_ocorrencias_cliente ON ocorrencias(cliente_id);
CREATE INDEX idx_ocorrencias_veiculo ON ocorrencias(veiculo_id);
CREATE INDEX idx_ocorrencias_status ON ocorrencias(status);
CREATE INDEX idx_ocorrencias_data_quebra ON ocorrencias(data_quebra);
CREATE INDEX idx_ocorrencia_logs_ocorrencia ON ocorrencia_logs(ocorrencia_id);
...
INSERT INTO usuarios (nome, email, senha, cargo, perfil) VALUES
('Administrador', 'admin@sistemacco.com', '$2a$10$xZKhvXQqh5L5L5L5L5L5LueYqBqvqvqvqvqvqvqvqvqvqvqvqvqvq', 'Administrador', 'administrador');

-- Tipos de quebra padrão
INSERT INTO tipos_quebra (nome, descricao) VALUES
('Suspensão', 'Problemas relacionados à suspensão do veículo'),
('Motor', 'Problemas relacionados ao motor'),
('Elétrica', 'Problemas no sistema elétrico'),
('Lubrificação', 'Problemas de lubrificação'),
('Freios', 'Problemas no sistema de freios'),
('Pneus', 'Problemas com pneus'),
('Outros', 'Outros tipos de ocorrências');

COMMENT ON DATABASE sistema_cco IS 'Sistema de Controle de Centro de Controle Operacional';
